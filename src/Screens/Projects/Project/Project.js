...
class Project extends React.Component {
  ...
  getProjectsImagesHandler(id){
    axios.get(this.state.url + 'api/project_child_images/' + id).then( res => {
      const projectImages = res.data;
      this.setState({projectImages: projectImages.result, allowToRender: false});
      console.log('AXIOS IN PROJECT '+ JSON.stringify(projectImages));
  ...
  render() {
    ...
    return (
      <BurgerMenu logo={this.state.logo}>
        ...
        {this.state.hideNav ? (
          <MobileScreen
            {...this.state.allowToRender ? this.getProjectsImagesHandler(projectid) : null}
